...
export const JournalView: React.FC<JournalViewProps> = ({ onTriggerCoach }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl p-5 sm:p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 shadow-celestial border border-white/80">
        <div className="flex items-center space-x-3">
          <button
            ...
            className="p-2 rounded-2xl border border-indigo-100 text-slate-600 hover:bg-white hover:text-indigo-600 transition-all shadow-sm"
          >
          ...
          <div>
            <div className="flex items-center space-x-2">
              <input
                ...
                className="font-extrabold text-xl text-slate-900 bg-transparent border-0 focus:ring-2 focus:ring-indigo-500 rounded-lg p-0 cursor-pointer"
              />
              {isToday && (
                <span className="text-[10px] font-bold uppercase tracking-wider bg-indigo-600 text-white px-2.5 py-0.5 rounded-full shadow-sm">
                  Today
                </span>
              )}
            </div>
            <p className="text-xs text-slate-500 font-medium">Daily Intentions & Execution Journal</p>
          </div>
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={handleSave}
            className="flex items-center space-x-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-full text-xs font-bold shadow-md shadow-indigo-500/25 transition-all"
          >
      ...
      {saveStatus && (
        <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 text-emerald-950 text-xs font-semibold px-4 py-2.5 rounded-2xl flex items-center justify-between shadow-sm animate-fadeIn">
      ...
      {activeTab === 'morning' ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-1.5">
                    1. Top Essential Priority for Today
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full text-sm px-4 py-3 rounded-2xl border border-indigo-100 focus:ring-2 focus:ring-indigo-500 bg-white/80 shadow-sm font-medium"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full text-sm px-4 py-3 rounded-2xl border border-indigo-100 focus:ring-2 focus:ring-indigo-500 bg-white/80 shadow-sm resize-none font-medium"
                  />
                </div>
            ...
            <div className="glass-panel rounded-3xl p-6 sm:p-7 space-y-4 shadow-celestial border border-white/80">
              ...
              <div className="space-y-2.5">
                {tasks.length === 0 ? (
                  <p className="text-xs text-slate-400 italic py-2">No tasks added yet for today. Add your critical actions below.</p>
                ) : (
                  tasks.map((task, idx) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center space-x-3 flex-1 min-w-0 mr-3">
                          ...
                          <span className={`text-sm truncate ${task.completed ? 'line-through' : 'font-semibold'}`}>
                            {task.text}
                          </span>
                          {linkedGoal && (
                            <span className="text-[10px] font-bold bg-gradient-to-r from-indigo-50 to-purple-50 text-indigo-700 px-2.5 py-0.5 rounded-full border border-indigo-200 flex-shrink-0">
                              {linkedGoal.title}
                            </span>
                          )}
                        </div>
              ...
              <div className="pt-2 flex flex-col sm:flex-row items-center gap-2">
                <input
                  ...
                  className="flex-1 text-sm px-4 py-2.5 rounded-2xl border border-indigo-100 focus:ring-2 focus:ring-indigo-500 bg-white/90 shadow-sm w-full"
                />

                <select
                  value={newTaskGoalId || ''}
                  onChange={(e) => setNewTaskGoalId(e.target.value ? Number(e.target.value) : undefined)}
                  className="text-xs px-3.5 py-2.5 rounded-2xl border border-indigo-100 bg-white text-slate-700 w-full sm:w-auto shadow-sm"
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto bg-gradient-to-r from-indigo-50 to-purple-50 hover:from-indigo-100 hover:to-purple-100 text-indigo-900 border border-indigo-200 px-4 py-2.5 rounded-2xl text-xs font-bold flex items-center justify-center space-x-1 transition-all shadow-sm"
                >
          ...
          <div className="space-y-6">
            <div className="glass-panel rounded-3xl p-6 sm:p-7 space-y-5 shadow-celestial border border-white/80">
              <h3 className="font-extrabold text-slate-900 text-base">Morning Vitals</h3>
              ...
              <div>
                <label className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-2 flex items-center justify-between">
                  ...
                  <span className="text-indigo-700 font-mono font-extrabold">{log.sleep_hours || 7.0} hrs</span>
                </label>
                <input
                  type="range"
                  min="0"
              ...
              {/* Sleep Quality (1-5) */}
              <div>
                <label className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-2 flex items-center space-x-1.5">
              ...
              {onTriggerCoach && (
                <div className="pt-3 border-t border-indigo-100/60">
                  <button
                    ...
                    className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-amber-50 via-rose-50 to-indigo-50 hover:from-amber-100 hover:to-indigo-100 text-indigo-950 border border-indigo-200/80 py-3 rounded-2xl text-xs font-extrabold transition-all shadow-sm"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full text-sm px-4 py-3 rounded-2xl border border-indigo-100 focus:ring-2 focus:ring-indigo-500 bg-white/80 shadow-sm resize-none font-sans font-medium"
                  />
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="glass-panel rounded-3xl p-6 sm:p-7 space-y-5 shadow-celestial border border-white/80">
              <h3 className="font-extrabold text-slate-900 text-base">Evening Vitals</h3>

              {/* Deep Work Hours */}
              <div>
                <label className="block text-xs font-extrabold text-slate-700 uppercase tracking-wider mb-2 flex items-center justify-between">
                  <span>Deep Work Blocks</span>
                  <span className="text-indigo-700 font-mono font-extrabold">{log.deep_work_hours || 0.0} hrs</span>
                </label>
                <input
                  type="range"
                  min="0"
              ...
              {/* Workout toggle */}
              <div className="pt-2">
                <label className="flex items-center space-x-3 cursor-pointer p-3.5 rounded-2xl border border-indigo-100 bg-white/80 hover:bg-white transition-all shadow-sm">
                  <input
                    type="checkbox"
                  ...
                  />
                  <span className="text-xs font-bold text-slate-800">Workout Completed</span>
                </label>
              </div>
              ...
              {onTriggerCoach && (
                <div className="pt-3 border-t border-indigo-100/60">
                  <button
                    ...
                    className="w-full flex items-center justify-center space-x-2 bg-gradient-to-r from-indigo-50 to-purple-50 hover:from-indigo-100 hover:to-purple-100 text-indigo-950 border border-indigo-200/80 py-3 rounded-2xl text-xs font-extrabold transition-all shadow-sm"
                  >
