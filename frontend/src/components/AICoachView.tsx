...
export const AICoachView: React.FC<AICoachViewProps> = ({ initialMode = 'morning' }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-3xl p-6 sm:p-7 shadow-celestial border border-white/80 relative overflow-hidden">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">AI Coach Studio</h2>
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center justify-center space-x-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-5 py-2.5 rounded-full text-xs font-semibold shadow-sm transition-all"
          >
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-2xl border text-left transition-all ${
                  isSelected
                    ? 'bg-gradient-to-br from-white via-indigo-50/60 to-purple-50/50 border-indigo-300 ring-1 ring-indigo-300 shadow-sm'
        ...
        {mode === 'goal-alignment' && (
          <div className="mt-3.5 pt-3 border-t border-indigo-100/60 flex items-center space-x-2.5">
            <span className="text-xs font-semibold text-slate-700">Target Goal:</span>
            <select
              value={selectedGoalId || ''}
              onChange={(e) => setSelectedGoalId(Number(e.target.value))}
              className="text-xs px-3 py-1.5 rounded-xl border border-indigo-100 bg-white/90 text-slate-800 shadow-sm font-medium"
            >
              {goals.map((g) => (
      ...
      {/* Error state */}
      {error && (
        <div className="bg-rose-50 border border-rose-200 text-rose-900 p-3.5 rounded-2xl flex items-start space-x-2.5 text-xs shadow-sm">
      ...
      {loading && (
        <div className="glass-panel rounded-3xl p-10 text-center space-y-3 shadow-celestial border border-white/80">
          <div className="w-12 h-12 rounded-2xl bg-indigo-600 flex items-center justify-center mx-auto text-white shadow-sm animate-bounce">
      ...
      {coachingResult && (
        <div className="space-y-6">
          ...
          <div className="glass-panel rounded-3xl p-6 sm:p-7 relative overflow-hidden shadow-celestial border border-indigo-200/80">
            <div className="flex items-center space-x-2 text-xs font-semibold text-indigo-700 mb-2">
            ...
            {coachingResult.why_this_rule && (
              <p className="text-xs text-slate-700 mt-3 bg-white/80 p-3.5 rounded-xl border border-indigo-100 shadow-sm leading-relaxed">
                <strong className="text-indigo-950 font-semibold">Why this matters: </strong>
                {coachingResult.why_this_rule}
              </p>
            )}
            ...
            {(coachingResult.next_action || coachingResult.immediate_action) && (
              <div className="mt-3 flex items-center space-x-2.5 bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 text-emerald-950 p-3.5 rounded-xl text-xs font-medium shadow-sm">
            ...
            <div className="mt-5 pt-3 border-t border-indigo-100/60 flex flex-wrap items-center justify-between gap-2 text-xs">
              <div className="flex items-center space-x-2">
                <span className="text-slate-500 font-normal">Source:</span>
                <span className="font-mono font-semibold bg-indigo-50 text-indigo-700 px-2 py-0.5 rounded-md border border-indigo-200/70">
                  {coachingResult.source || 'agent_coach'}
                </span>
                {coachingResult.confidence && (
                  <span className="font-mono text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-md border border-emerald-200 font-semibold">
          ...
          <div className="glass-panel rounded-3xl p-5 shadow-celestial border border-white/80">
            ...
            <pre className="bg-white/90 text-slate-800 p-3.5 rounded-xl text-xs font-mono overflow-x-auto border border-indigo-100 max-h-72 shadow-inner">
